...
Download_ocserv(){
	mkdir "ocserv" && cd "ocserv"
        wget "https://raw.githubusercontent.com/fctx/one-oc/main/ocserv-1.1.6.tar.xz"
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/fctx/one-oc/master/ocserv.conf"
...
Set_passwd(){
	...
	read -e -p "(默认: fengchigofengchigo):" userpass
	[[ -z "${userpass}" ]] && userpass="fengchigofengchigo"
...
Add_iptables(){
	...
	iptables -I INPUT -p tcp --dport ${set_tcp_port} -j ACCEPT
	iptables -I INPUT -p udp --dport ${set_udp_port} -j ACCEPT
}
Del_iptables(){
        iptables -D FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
        iptables -D FORWARD -s 10.31.0.0/24  -j ACCEPT
	iptables -D INPUT -p tcp --dport ${tcp_port} -j ACCEPT
...
Set_iptables(){
	...
	if [[ -z ${ifconfig_status} ]]; then
        ...
        iptables -t nat -A POSTROUTING -s 10.12.0.0/24 -o ${Network_card} -j MASQUERADE
